.readBismark <- function(files, colData) {
    ...
    for (i in seq_along(files)) {
        ...
        cat(paste("Processing sample ", rownames(colData)[
            i], " ... \n", sep = ""))
        ...
        methData[[i]] = GRanges(
            seqnames = bismark[[1]], ranges = IRanges(start = bismark[[
                2]], width = 1), methylated = bismark[[5]], reads = bismark[[
                    5]] + bismark[[6]] )
        rm(bismark)
    }
    ...
    tReads <- matrix(integer(length = length(
        fData) * length(methData)), nrow = length(fData))
    mReads <- matrix(integer(length = length(
        fData) * length(methData)), nrow = length(fData))
    ...
    BSDAT = cBSDMC(
        rowRanges=fData,methReads=mReads,totalReads=tReads,methLevels=lReads,
        colData = colData)
...
setMethod("readBismark", signature(
    files = "character", colData = "DataFrame"), .readBismark)

#' @rdname readBismark-method
#' @aliases readBismark-method readBismark
setMethod("readBismark", signature = c(
    files = "character", colData = "data.frame"), function(
        files, colData) {colData = as(
            colData, "DataFrame")
...
#' @aliases readBismark-method readBismark
setMethod("readBismark", signature = c(
    files = "character", colData = "character"), function(
        files, colData) { colData = DataFrame(
            row.names = colData)
        .readBismark(files, colData) })
