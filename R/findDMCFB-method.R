...
.MCMCFB<-function(object,bwa,bwb,nBurn,nMC,nThin, sdv, nCores, pSize, sfiles){
    if(missing(object)){
    ...
    if(missing(sdv)){
        sdv = sqrt(mean(rowVars(methReads(
            object)/totalReads(object)), na.rm = TRUE))
        cat(paste0(
            "The priors's SD = ", round(sdv,4), ", estimated from data ... \n"))
    }
    ...
    if(missing(nCores)){
        ...
        nCores = floor(maxram/(5*nBurn*nThin*pSize*bwa*bwb/10000000))
        nCores = max(1, min(nCores, multicoreWorkers()))
        }
    }

    cat(paste0("Number of assigned cores: ", nCores,  " ... \n"))
    ...
    formula = as.formula(paste("logit(MethRead/ReadDepth)", paste(
        c(names(colData(object))), collapse = " + "), sep = " ~ "))

    cat("------------------------------------------------------------\n")
    cat(paste0("Fitted model: "))
    if(length(c(names(colData(object))))>1){
        formula2 = as.formula(paste0("logit(MethRead/ReadDepth) ~ ",paste0(
            "F(", c(names(colData(object)))[1],") + "), paste(
                c(names(colData(object)))[-1], collapse = " + ")))
        cat(paste(formula2[2], formula2[1],formula2[3], sep = " "),"\n")
    }else{
        formula2 = as.formula(paste0("logit(MethRead/ReadDepth) ~ ",paste0(
    ...
    cat("------------------------------------------------------------\n")
    cat(paste0("Creating ", length(
        Pos.list1), " batches of genomic positions ... \n"))
    ...
    for(indK in seq_along(Pos.list1)){
        ...
        cat(paste0("Running batch ",sprintf(paste0("%",nchar(trunc(
            length(Pos.list1))),"d"), indK), "/", length(
                Pos.list1), "; ", paste0( (seqnames(object.par))@values), "; ",
            sprintf(paste0("%",nchar(trunc(aaa)),"d"),dim(
                object.par)[1]), " positions; Region [",
            sprintf(paste0("%",nchar(trunc(
                tail(Pos.list1[[length(Pos.list1)]])[6])),"d"),
                start(object.par)[1]),", ", sprintf(paste0("%",nchar(trunc(
                    tail(Pos.list1[[length(Pos.list1)]])[6])),"d"), tail(
                        start(object.par))[6]) , "]; Date ", paste0(
                            Sys.time()), "\n"))
        ...
        for(i in seq_along(Pos.list)){
            mylist[[i]] = list(object=object.par[
        ...
        .fitpar <- function(i) {
            ...
            formula = as.formula(paste("logit(MethRead/ReadDepth)", paste(
                c(names(colData(object.par2))), collapse = " + "), sep = " ~ "))
            object.par2.df <- as.data.frame(colData(object.par2))
            object.par2.df <- object.par2.df[all.vars(
                formula)[all.vars(formula) %in% names(colData(object.par2))]]
            ...
            nz=2+sum(vapply(object.par2.df.factor, nlevels, numeric(1))[-1]-1)+
            ...
            suppressWarnings(fit0 <- try(
                glm(cbind(data.all[,1],data.all[,2]-data.all[
                    ,1])~offset(GammaDeltaEta.mean+Beta.mean)-1,family=
                        'binomial'), silent = TRUE))
            ...
            for(myiter in seq_len(nits)){
                ...
                suppressWarnings(fitg <- try(
                    speedglm(cbind(Y,m-Y)~offset(
                        Beta.mean)-1+Xgde,data=gdata, family=binomial(
                            logit)) , silent = TRUE))
                options(show.error.messages = TRUE)
                ...
                suppressWarnings(mle.Prec <- try(
                    ginv(mle.Sig), silent = TRUE))
                suppressWarnings(Post.Cov <- try(
                    ginv(mle.Prec+GDEPrec), silent = TRUE))
                options(show.error.messages = TRUE)
                ...
                for(i in seq_len(nseq)){
                    ...
                    suppressWarnings(fiti <- try(
                        speedglm(cbind(Y, m-Y) ~ offset(
                            GammaDeltaEta.mean)-1+ Xns2, data=idata,
                            family=binomial(logit)), silent = TRUE))
                    options(show.error.messages = TRUE)
                    ...
                    suppressWarnings(Post.Cov <- try(
                        ginv(mle.Prec + BPrec), silent = TRUE))
                    options(show.error.messages = TRUE)
                ...
                if ((is(fit0)[1] == "try-error") ) {
                    ...
                    fit0 = bayesglm(cbind(data.all[,1], data.all[,2] - data.all[
                        ,1]) ~ offset(GammaDeltaEta.mean + Beta.mean) - 1,
                        family = binomial(logit), maxit = 1)
                    options(warn=0)
        ...
        tmpf = tempfile(pattern=paste0("object.part",indK,"-"),fileext=".rds")
        saveRDS(totres, tmpf)
        tmpfiles[[indK]] = tmpf
        rm(totres)
    }
    ...
    cat("------------------------------------------------------------\n")
    cat(paste0("Combining ",length(tmpfiles), " objects ... \n"))
    object.new = NULL
    ...
    cat(paste0("Objects are combined.\n"))
...
.findFB <- function(object, alpha){
    ...
    object.df <- object.df[all.vars(formula)[all.vars(
        formula) %in% names(colData(object))]]
    ind1 <- vapply(object.df, is.character, logical(1))
    object.df[ind1] <- lapply(object.df[ind1], as.factor)
    ind1 <- vapply(object.df, is.factor, logical(1))
    object.df.factor <- as.data.frame(object.df[, ind1])
    ...
    for (mm in seq_along(MClist)) {
        gfit = list()
        for (k in seq_len(dim(Cmb)[2])) {
            gfit[[k]] = apply(MClist[[mm]]$GMAT[[k]], 2, quantile, probs = c(
    ...
    if(length(MClist[[1]]$gfit)>2){
        gfit.pairs = lapply(MClist, function(elt){elt$gfit})
        gfit.pairs = lapply(gfit.pairs, function(x) {do.call("rbind", x[-1])})
        ...
        DMCs.pairs = matrix(rep(c(0,1), (dim(gfit.pairs)[1]/3)*(
            dim(gfit.pairs)[2])), nrow = dim(gfit.pairs)[1]/3, ncol = dim(
                gfit.pairs)[2])
        DMCs.dir = matrix(rep(c("Equal","Hyper","Hypo"),(dim(gfit.pairs)[1]/3)*(
            dim(gfit.pairs)[2])), nrow = dim(gfit.pairs)[1]/3, ncol = dim(
                gfit.pairs)[2])
        DMCs = matrix(rep(c(0,1),dim(gfit.pairs)[2]), nrow = dim(gfit.pairs)[
            2], ncol = 1)
        for (j in seq_len(dim(gfit.pairs)[1]/3)) {
            DMCs.pairs[j,] = as.numeric(gfit.pairs[(j-1)*3+1,]* gfit.pairs[
                (j-1)*3+3,] > 0)
        ...
        DMCs.dir = matrix(rep(c("Equal","Hyper","Hypo"), (dim(
            gfit.pairs)[2])), nrow = dim(gfit.pairs)[2], ncol = 1)
        j=1
        DMCs[,j] = as.numeric(gfit.pairs[(j-1)*3+1,]*gfit.pairs[(j-1)*3+3,] > 0)
    ...
    np = paste(combn(levels(object.df.factor[,1]), 2)[2, ], combn(
        levels(object.df.factor[,1]), 2)[1, ], sep = "vs")
    namesPairCon = paste0(rep(c(paste("Cont", np, sep = "")
    ), each = 3),colnames(gfit.pairs))
    ...
    bhat = t(do.call("cbind", lapply(MClist, function(elt){elt$bhat})))
    logitbstar = t(do.call("cbind", lapply(MClist, function(elt){elt$bstar})))
    expit=function(x){return(1/(1+exp(-x)))}
    names(bhat) = colnames(object)
    bstar = expit(logitbstar)
    colnames(bstar) = colnames(object)

    object.new = cBSDMC(rowRanges = rowRanges(object),
...
.findDMCFB <- function(
    ...
    if(!(c("MClist")%in%names(metadata(object)))){
        cat("------------------------------------------------------------\n")
        cat(paste0("Running Bayesian functional regression model ...\n"))
        object = .MCMCFB(
            object, bwa, bwb, nBurn, nMC, nThin, sdv, nCores, pSize, sfiles)
    }
    cat("------------------------------------------------------------\n")
    cat(paste0("Identifying DMCs ...\n"))
    object = .findFB(object, alpha)
    cat(paste0("DMCs are identified.\n"))
    SNAM = paste0(
        "BSDMC-pSize",pSize,"bwa",bwa,"bwb",bwb,"nBurn",nBurn,"nMC",nMC,"nThin",
        nThin,"alpha",alpha,".rds")
    if(sfiles){
        saveRDS(object, paste0(SNAM))
        cat(paste0(
            "BSDMC object (includes DMCs) stored as '",  paste0(SNAM), "'.\n"))
    }
    cat("------------------------------------------------------------\n")
    res = table(factor(rowData(object)[,1], levels = c(0,1)))/dim(object)[1]*100
    names(res) <- c("Equal(%)", "DMC(%)")
    cat(paste0("Percentage of non-DMCs and DMCs:\n"))
    print(res)
    cat("------------------------------------------------------------\n")
    cat(paste0(
    ...
    if((dim(rowData(object))[2]-1)/5 ==1){
        res = data.matrix(100*table(factor(unlist(rowData(object)[(2+(dim(
            rowData(object))[
                2]-1)/5):(1+2*(dim(rowData(object))[
                    2]-1)/5)]), levels = c(1,2,3)))/dim(object)[1])
        ...
        res = 100* vapply(rowData(object)[(2+(dim(rowData(object))[
            2]-1)/5):(1+2*(dim(rowData(object))[
                2]-1)/5)], table, FUN.VALUE=numeric(3L))/dim(object)[1]
        rownames(res) =c("Equal(%)","Hyper(%)","Hypo(%)")
    }
    print(t(res))
    cat("------------------------------------------------------------\n")
